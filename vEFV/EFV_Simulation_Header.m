...
for l = [60,375,535,750]
    for w = [4,8]
        ...
        ans  =struct('L',l/(10^6),'L0',l/(10^6),'th',th,'w',(w-1.5)/(10^6),...
            'A',th*(w-1.5)/(10^6),'I',th.^3*(w-1.5)/(10^6)/12,'type','r');
        eval([ v '=ans;']);
        %Geomtry w triangles
        v = genvarname(sprintf('efv%dx%d',l,w));
        ans  =struct('L',l*1.12/(10^6),'L0',l/(10^6),'th',150e-9,'w',(w-1.5)/(10^6),...
            'A',th*(w-1.5)/(10^6),'I',th.^3*(w-1.5)/(10^6)/12,'type','efv');
        eval([ v '=ans;']);
        %Geomtry w stubs
        v = genvarname(sprintf('n%dx%d',l,w));
        ans  =struct('L',l*3/(10^6),'L0',l/(10^6),'th',150e-9,'w',(w-2)/(10^6),...
            'A',th*(w-2)/(10^6),'I',th.^3*(w-2)/(10^6)/12,'type','n');
        eval([ v '=ans;']);
    end
end
